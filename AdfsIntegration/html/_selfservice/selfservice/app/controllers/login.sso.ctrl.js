(function () {
	...
	LoginSSOController.$inject = ['$http', '$scope', '$rootScope', 'SWSessionService', '$state', 'store', 'RequestService', 'wssLogging', '$location', '$window', '$cookies'];

	function LoginSSOController($http, $scope, $rootScope, SWSessionService, $state, store, RequestService, wssLogging, $location, $window, $cookies) {
		...
		$scope.login = ( $scope.sessServ.sspConfig.type === 'saml' ? function () {
				var loc = $location.absUrl().split('/');
			    $window.location.href = $scope.sessServ.sspConfig.serverAddress + ((loc.splice(-2), loc).slice(3).join('/') + '/sso/saml_auth.php') + '?' + 'wssinstance=' + encodeURIComponent($scope.sessServ.sspConfig.selfServiceInstance) + '&returnto=' + encodeURIComponent(loc.join('/'));
		} : function () {
			$http({
		...
		if ($scope.sessServ.ssoEnabled === true) {
			if ($scope.sessServ.previousLogin === false) {
				...
				var cred = JSON.parse(atob($cookies.get('auth')));

				if (angular.isDefined(cred.error)) {
					//-- Authentication error. Possibly anonymous logon, incorrect cutomer password or incorrect rights
					...
					$scope.sessServ.username = cred.custid;
					$scope.sessServ.sessionId = cred.sessionId;
					$scope.sessServ.sessionConfig = cred;
					$scope.sessServ.bindSession($scope.sessServ.sessionId).then(function () {
						$scope.sessServ.getSelfServiceConfig($scope.sessServ.username).then(function () {
							$scope.sessServ.wssConfig = store.get("wssConfig");
							$scope.sessServ.getCustomerDetails(cred.custid, $scope.sessServ.wssConfig.ac_id).then(function (custDetails) {
								//Get number of authorisations, stick it in rootScope
								RequestService.getAuthCount().then(function (response) {
									$scope.sessServ.numAuths = response;
									if ($rootScope.goToPath !== '/' && $rootScope.goToPath !== '/login' && $rootScope.goToPath !== '/loginsso' && $rootScope.goToPath !== '/loginmanual') {
